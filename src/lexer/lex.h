...
typedef struct {
    ...
    char current_file[TOKEN_LENGTH]; // The name of source file we are reading
                                     // from.
    char buffer[1];                  // A buffer so that chars can be "put back"
    long position; // The posistion of the file pointer in the current file in
...
// Wrapper for getc. Takes a lexer pointer and returns the next character in the
// file its holding onto. Updates position and stream
int lexer_getchar(Lexer *l);

// Wrapper for ungetc. Takes a lexer pointer and back-tracks 1 character using
// the lexer buffer. Updates position
